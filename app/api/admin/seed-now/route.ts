/**
 * Quick Seed API Route
 * Generates and inserts 20 high-quality scholarships using Groq AI
 * Admin-only route - can be called directly
 */
...
Return ONLY valid JSON, no markdown, no code blocks.`;
        ...
        errors.push(`${template.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to seed global scholarships',
